...
extract_DP <- function(pdx_value) {
  pdx_parts <- str_split(pdx_value, ":")[[1]]
  af_value <- as.numeric(pdx_parts[4])
  return(af_value)
}

df2 <- df2 %>%mutate(DP = sapply(PDX, extract_DP))
df2 <- df2 %>%mutate(Germline_DP = sapply(GL, extract_DP))
...
finaldf <- finaldf %>%
  ...
  mutate(DP_mean = mean(DP)) %>%
  ungroup()

############
finaldf <- finaldf %>% distinct(UNIQMUT, Protocol, Sample,DP_mean)
finaldf <- finaldf %>% mutate(Protocol = fct_relevel(Protocol, "Unsorted"))


mean_scores <- aggregate(DP_mean ~ Sample, data = finaldf, FUN = mean)
mean_scores <- mean_scores[order(mean_scores$DP_mean, decreasing = F), ]  
...
# Create the box plot
plot1 <- ggplot(finaldf, aes(x=Sample, y=DP_mean, fill=Protocol)) +
  geom_boxplot(outlier.shape = NA) + 
  geom_point(position = position_jitterdodge(), alpha=0.5, size=0.8)+
  labs(y = "Depth (coverage) ", x = "PDX") +
  #scale_y_continuous(limits = c(0, 1500), breaks = seq(0, 1500, by = 100))+
  scale_y_log10()+
  annotation_logticks(sides = 'l') +theme(panel.grid.minor = element_blank(), panel.grid.major = element_blank())+
  stat_compare_means(aes(group = Protocol), label = "p.signif", label.y = 4, size = 5)+
  theme_classic()+
